...
5. **不回归**：聚焦 336 passed（AVX2 + layout + packed + W8A8 AVX2 + Qwen35 AVX2 parity + vector-common）；全量 `python/tests/ut/pypto_x` **1527 passed / 7 skipped / 0 failed**（13:47，commit `4e52610f3`）；收集数 base 1400 → head **1534**（+134，唯一消失的 base 用例是刻意的边界改名）。
...
```text
位级:     python/tests/ut/pypto_x/test_cpu_avx2_layout.py（134 例，纯 stdlib oracle 主口径）
...
聚焦:     logs/focused-*.log（336 passed）
...
```text
聚焦:  336 passed（logs/focused-336.log）
全量:  1527 passed / 7 skipped / 0 failed in 827.52s（logs/full-pytest-after.out）
收集:  base 1400 → head 1534（+134）；逐名 diff 只有新增 134 个 + 1 个刻意改名
