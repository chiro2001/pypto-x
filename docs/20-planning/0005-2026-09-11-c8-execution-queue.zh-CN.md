...
## 10. L5/L6 决策包（供用户裁决；决定 Q6 能否启动）

契约 `0002` 的 `D5` 明确：L4–L6 阈值需要用户确认。L4 已在跑（Q3 出结果、C7 查缺口），但 **L5/L6 至今没有语料与阈值**，因此：

- **不能**对外声称"W8A8 精度达标"（只能说"实现忠于契约方案、与同方案参照一致到 band 口径"）；
- **Q6**（长序列与 perplexity）无法启动。

### 10.1 L5（生成行为）—— 契约 §5.2 的暂定口径

```text
指标：greedy decode 的 top-1 一致率 ≥ 99%（128 token）、top-5 一致率 ≥ 99.9%、首次分叉 ≥ 32 token
语料：不需要额外语料，用现成 prompt 集合
成本：128 token × greedy，CPU 上以 0.8B 模型量级估算为分钟级（本机）；A3 更快
现状：我们只跑过 4–5 步 greedy（Q3 的 token 链），**远未达到 128 token 的口径**
```

**建议**：L5 用**现有三 prompt 做弱化版**（例如 32 token）并**显式标注覆盖不足**；等 L6 语料定了再合并做正式版。这样不至于为了"跑满 128 token"单独占一个机时窗口。

### 10.2 L6（perplexity）—— 需要你定的两件事

**① 语料**（三选一）

| 选项 | 说明 | 成本 | 风险 |
|---|---|---|---|
| **A. 标准小语料**（如 wikitext-103 的一个子集、或 C4 的若干片段） | 可与外部结果对照；**需要在 A3 或本机下载**（A3 侧可用 NJU 镜像；不含 LLM 权重，属普通数据集） | 下载 + 前向：数十万 token 量级在 A3 上可接受 | 需要你点头"允许下载一个公开文本语料" |
| **B. 用户提供的语料** | 最贴近真实用途 | 取决于你给的规模 | 需要你提供文件 |
| **C. 复用我们的 prompt 集合** | 零成本 | 极低 | **样本太小、且与调参同源**，ppl 数字几乎没有意义 → **不建议**作为正式口径 |

**② 阈值**（契约暂定 `Δppl ≤ +1.0%` 或 `+0.2` 绝对值）

- 这个量级对应"W8A8 量化后语言建模能力几乎无损"，属于**偏严**的口径；
- 若选 A 语料，建议先**只测不判**（把 W8A8 与 BF16 的 ppl 都跑出来），**再**据实测分布定阈值——避免先定阈值后被数字绑架；
- 若你要一次性给一个数，我建议先按**暂定值**跑，若明显达不到，再单独讨论是"实现问题"还是"量化本身的损失"（这两者要靠 C8 的 band 口径区分，不能混）。

### 10.3 需要你回的三句话

```text
L6 语料：A（标准小语料，需批准下载）/ B（你提供）/ C（复用 prompt，仅作参考）
L6 阈值：按暂定 +1.0%/+0.2 先跑后判  ／ 或你直接给一个数
L5 强度：先做弱化版（32 token，标注覆盖不足） ／ 或直接做正式版（128 token）
```

回完这三句，Q6 就能排进队列；同时"W8A8 精度是否达标"这句话才有依据。
