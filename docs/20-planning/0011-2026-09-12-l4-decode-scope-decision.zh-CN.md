...
| **C（维持现门）** | decode 维持 6/6 FAIL；需要先按 C7 建议做**最小增量实验**（把 zh/default decode step2 的输入 state 固定为 gold，逐层对比中间张量）把 56% 的 compute 缺口下钻到算子，再决定是否改实现 | 增量诊断 +（可能的）实现修复，工作量未估；C7 显示无单一热点，修复性价比待评估 | 只有在你需要"现门必须过"时才走这条路 |
...
3. 选项 C：派"decode step2 state 固定 + 逐层中间张量对比"的最小实验任务（预计一次锁窗）；
