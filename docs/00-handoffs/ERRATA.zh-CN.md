...
## ERR-0015（2026-09-19）：两处"计数口径"错误被独立验收发现并订正（总量必须逐项复算，不能沿用估计）

**状态**：已就地订正（`0022` §8 与 `0021` §3.3/§5），结论不受影响。

### 两处订正

1. **0022 §8 对比表"其他 13 类"写 254，实为 411**：`verify-0050-ops-batch2` 逐类相加独立复算（与作者 JSON 一致）得到
   其余 13 类缺失引用合计 **411**；19→13 类、六算子归零的结论不受影响。
2. **0021 的"unplannable 205→0"实为 212→0**：`verify-0049-u5-region-r4` 在 `8b5246557`（batch-2 已落地、scalar 委托前）
   实测 `unplannable_operations = 212`（`add:scalar` 176 + `mul:scalar` 29 + `where:scalar` 7）。
   作者把 `where` 的 7 次另列，故文中出现 205；round-4 后精确为 **212 → 0**。

### 教训

1. 任何跨表的"合计"必须**逐项复算并核对两类口径**（例如"缺失 contract 引用数" vs "unplannable 操作数"、
