...
| RTX 5080（`192.168.101.5`） | 在线；GPU 由 PyPTO-X 独占；M1K-CUDA PASS | NVIDIA CUDA/PTX/NVVM 验证；**2026-09-13 CUDA 线重启（batch 0045）全部 PASS**：C1 smoke + 扩展验收 10/10 在真 5080 上逐值 bit-exact（sm_120 cubin 4/4 位级一致）、本机 7 条设备 skip 的 CUDA 单测真机 **41 passed/0 failed/0 skipped**、cuBLAS GEMM 复核与冻结值差 ≤1.46%（同 616.92 窗口噪声）、W8A8 CUDA 真机差分 **53/53 checks 0 mismatch** | WSL2、RTX 5080 16,303 MiB、CC 12.0；2026-09-10 复测 KMD **616.92** / CUDA UMD **13.4**（文档旧值 610.62/13.3，已漂移）；Driver/PTX 连续两次执行371-input/51-output external-buffer 24层图 | GPU-only 无需 `gamepc` 锁；host-heavy 才持锁。**工具链已于 2026-09-13 实测确认可用**：`nvcc` 13.3.73（`/usr/local/cuda-13.3`）+ cuBLAS（`cuda-toolkit-wsl` 任务装入，10.7 GB，lock 中 `cuda-toolkit-wsl: complete`），此前"无 nvcc/cuBLAS"是 2026-09-09 探测的过期条目（见 ERR-0012）；cuBLAS 13.6.0.2 已被 `w8b_b3b_cuda_gemm_baseline` 用作 T1 基线并 r3 独立验收 PASS；PyTorch/Triton 仍无（如需另议）；**该机是用户共用的游戏机（2026-09-13 用户明确）**：若正在运行游戏，性能测量会不准——性能腿前后必须用 `nvidia-smi` 取证（utilization/clocks/power/`--query-compute-apps`），发现外来负载（无我们的计算进程却持续高利用率/高时钟/高功耗）就**跳过该轮性能测量**并在证据里写 `perf deferred: foreign GPU load detected`，绝不动用户的进程；正确性/编译类结论不受影响；空闲时钟约 427–487 MHz vs 峰值 3090 MHz，测性能前必须 warmup 并断言时钟；当前是 correctness kernel，不是 fusion/性能结论 |
...
| A3（用户借用的共享机） | **可用**（2026-09-11 用户借入）；**共享机 → 安静使用**；工作环境为自建容器；**2026-09-13 实测复核**：可达、640 核，但**当前有他人负载**（root 的 vLLM 服务 `/home/models/DeepSeek-V4-Flash-0731-w4a8` 端口 8150 + 其他用户作业，load ≈161/640）——所以"A3 空闲"不是常量：**每次用前先查 load 与我们的 CPU/NPU 窗口**，性能腿只在与 GamePC 同等的"干净窗口"跑，正确性腿不受影响；我方无遗留进程（`~/pypto-x-a3/*` 完好） | 原生 AArch64/SVE256 功能验收（替代已释放的 920B ECS）；昇腾 910C NPU 验证与基线 | 鲲鹏 920B CPU：aarch64，**SVE VL=32（256 位）原生**，含 `svebf16`/`svei8mm`/`svef32mm`/`svef64mm`；昇腾 910C NPU（CANN 9.1.0 + driver/firmware；容器内 torch 2.10 / torch_npu 2.10 / vllm 0.27.1）；640 核 / 2 TB 内存 / `/home` 13 TB | **CPU 固定最后一个 NUMA node**；**NPU 只用 chip7**（其余预留给用户的量化任务）；**只写 home 与自己创建的容器**；不改宿主机配置、不碰他人容器/进程/数据；访问方式与端点只在本地私有侧，不进仓；与 A2 相互独立 |
