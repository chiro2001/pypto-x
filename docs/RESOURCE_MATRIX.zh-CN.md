...
| 昇腾 A2（910B3）租用环境 | 在线（HiDevLab）；容器 256 vCPU / 2014 GiB / 300 GB；CANN 9.0.0 + 驱动 25.2.0 | Ascend 真机验收：PTO-ISA NPU ST、CANN 样例、Ascend adapter 回归 | `npu-smi` 显示 1× 910B3、Health OK、HBM 3450/65536 MB；`acl.get_soc_name()=Ascend910B3`（需先 `source /usr/local/Ascend/ascend-toolkit/set_env.sh`）；容器内无 torch/pypto/numpy | 出网受限：HTTPS 仅 gitcode/pypi 可达、github 与一切 22 端口被封；无 `/dev/net/tun`、无 NET_ADMIN → **OpenVPN/反向 SSH 到自建中继都不可行**；访问走 <relay-ip> 反向隧道（平台 token 短期有效，仅作引导）；租用共享资源 → **约定串行** |
...
互相抢 2222 端口导致 5 s 抖动，用 `scripts/remote/a2_910b.sh --install-tunnel` 重启可保证只有一个。

**注意**：<relay-ip>（<relay-host>）上为此增开了**独立 sshd 实例** `<relay-ssh-service>.service`（配置文件 `/etc/ssh/sshd_config_443`），
