...
| RTX 5080（`192.168.101.5`） | 在线；GPU 由 PyPTO-X 独占；M1K-CUDA PASS | NVIDIA CUDA/PTX/NVVM 验证 | WSL2、RTX 5080 16,303 MiB、CC 12.0；2026-09-10 复测 KMD **616.92** / CUDA UMD **13.4**（文档旧值 610.62/13.3，已漂移）；Driver/PTX 连续两次执行371-input/51-output external-buffer 24层图 | GPU-only 无需 `gamepc` 锁；host-heavy 才持锁。**工具链已于 2026-09-13 实测确认可用**：`nvcc` 13.3.73（`/usr/local/cuda-13.3`）+ cuBLAS（`cuda-toolkit-wsl` 任务装入，10.7 GB，lock 中 `cuda-toolkit-wsl: complete`），此前"无 nvcc/cuBLAS"是 2026-09-09 探测的过期条目（见 ERR-0012）；cuBLAS 13.6.0.2 已被 `w8b_b3b_cuda_gemm_baseline` 用作 T1 基线并 r3 独立验收 PASS；PyTorch/Triton 仍无（如需另议）；空闲时钟约 427–487 MHz vs 峰值 3090 MHz，测性能前必须 warmup 并断言时钟；当前是 correctness kernel，不是 fusion/性能结论 |
