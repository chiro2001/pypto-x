...
| AMD GamePC 核显 `gfx1036` | 静态 C3 PASS；WSL/HIP runtime `BLOCKED_DEVICE`；6750GRE 暂缓 | AMDGPU LLVM static codegen，后续 HIP 真机功能 | C3 math/reduction/batched matmul与Qwen 4,532/4,532 static lowering通过；Windows OpenCL device `gfx1036`，WSL无 `/dev/kfd`/ROCm/HIP | 静态 LLVM/ELF与host oracle不等于HIP执行；wave/BF16/INT8/MFMA/WMMA unknown；不能代表6750GRE性能 |
