...
| A3（用户借用的共享机） | **可用**（2026-09-11 用户借入）；**共享机 → 安静使用**；工作环境为自建容器 | 原生 AArch64/SVE256 功能验收（替代已释放的 920B ECS）；昇腾 910C NPU 验证与基线 | 鲲鹏 920B CPU：aarch64，**SVE VL=32（256 位）原生**，含 `svebf16`/`svei8mm`/`svef32mm`/`svef64mm`；昇腾 910C NPU（CANN 9.1.0 + driver/firmware；容器内 torch 2.10 / torch_npu 2.10 / vllm 0.27.1）；640 核 / 2 TB 内存 / `/home` 13 TB | **CPU 固定最后一个 NUMA node**；**NPU 只用 chip7**（其余预留给用户的量化任务）；**只写 home 与自己创建的容器**；不改宿主机配置、不碰他人容器/进程/数据；访问方式与端点只在本地私有侧，不进仓；与 A2 相互独立 |
