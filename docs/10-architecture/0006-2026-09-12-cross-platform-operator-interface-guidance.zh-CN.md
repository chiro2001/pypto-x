...
> **report 单文档校验边界（2026-09-12，0042，含 1b/1c 修订；2026-09-13 report schema v3 内嵌真源）**：`validate_report_schema` **只接受 schema v3**；v2 及更早版本以 `report_schema_version_legacy_rejected` 明确拒绝（永不静默升级、也永不按"缺省 v3 检查"解释）。规范集（fail-closed）如下。
>
...
>
> **（四）不可自证的残余边界（完整清单；R1–R3 已闭合，R4/R5 已登记且各有已复现的最小反例）**：① 运行态/实测事实：`timing.*` 数值、`artifact.content_digest`、`dispatch.threads_used` 的具体值（只对账文档内副本一致性）、coverage 计数、`gates.*`；② 未被代码 pin 的 probe 事实：host capability `features`/target snapshot digest、AOCL fingerprint 的 `path`/`name`/`size_bytes`/`default_threads`/BLIS 版本后缀，以及 **vendor manifest 线程计数值**（`max_threads`/`threads.{max,default}_threads`/`library.default_threads`）：四者一致且为正的校验拒绝部分协调改写（verify-0043 `ce3_max_threads.py` 的反例现被 `report_provider_manifest_thread_facts_mismatch` 拒绝），但四者连同重算 `provider_artifact_digest`、decision library 副本与全部报告副本的全量改写仍被接受（数值本身无代码常量）；③ **plan payload 未内嵌**：`resolution.plan_digest`，以及 runtime-fallback 报告中 declared provider 的 `resolution_decision.document.selected_threads`/`selected_thread_mode`——协调改写 decision body + 重算 decision digest + 全部报告副本仍被接受（单副本/未重算 digest 被拒；verify-0043 `ce4_resolved_threads_fallback.py` 的反例现被 `report_resolution_decision_not_reconciled` 拒绝；无 runtime fallback 时线程数由内嵌 manifest + digest-checked `requested_policy` 重推，不再属于此边界）；④ **注册身份替换**：锚只证明内嵌文档是某个注册 provider 的 canonical manifest，并不证明实际运行的就是该身份——把整份报告（全部副本、两个内嵌文档与 digest）整体改写为另一个注册 provider 的完整自洽故事仍可通过；⑤ request dtype/shape 等文档内无外部真源字段的全副本协调改写；⑥ 说明层（见（三））中的位置型 payload、叙述字段、非 selected candidate echo 与线程环境账目。除以上六类外，报告内其余 manifest-backed 字段均已被（一）（二）对账或在协调改写下被注册身份锚/常量检查拒绝。因此 `validate_report_schema` **不是执行授权**：它在 `ExecutionReport.__init__` 中于执行**之后**运行，仓内没有任何执行路径消费 report，下游消费者不得据 report 重新规划。执行授权只以 `execute_matmul_plan` 对 digest-checked capability 的 canonical 重算（`entry._verify_plan_binding`）通过为准。
