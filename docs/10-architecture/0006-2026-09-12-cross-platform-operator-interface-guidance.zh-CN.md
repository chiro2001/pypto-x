...
`view_mode` 是约束/偏好，不是 alias 授权：contract 的 `view_obligation`、stride 可表达性、写后读、owner/lifetime 和多 view 重叠 proof 优先于 policy。`prefer` 只影响候选排序；`require` 要求 proof 必须通过，无法证明时以 `ALIAS_PROOF_UNAVAILABLE` 给出失败条件并 fail-closed，绝不静默拷贝。**2026-09-12 用户批准 `require` 提级为公共 stable API；同日独立验收判 PASS_WITH_BOUNDARIES（受检 tip `8fff50558`，含全量 UT 1674/1667/7/0/0、三条伪造反例全拒、对抗用例与逐位一致证据），**本节自 2026-09-12 起生效**。提级以证明链与执行器启用为前提，因此 `require` 的 stable 语义同时包含"proof 通过 → 零拷贝 view"与"proof 不通过 → 结构化失败（`ALIAS_PROOF_UNAVAILABLE`）"两半。**旧的 `allow_view` 只作为兼容输入，不能把 `must_not_alias` 改成 `may_alias`。

> **边界（2026-09-12 独立验收登记，2026-09-13 schema v4 修订后仍可接受）**：① `ExecutionPlan.validate` 是 **content-local 门**，通过它**不等于**执行授权；执行授权只以执行层对已注册 contract/capability 的 canonical 重算为准（全自洽的 obligation-swap plan 会在 provider 调用前被拒、输出无副作用）。② report 自 **schema v4** 起内嵌 plan 的 canonical payload（`resolution.plan_digest` 的 preimage，`resolution_decision` 由它确定性重推），并内嵌 invoked provider 的 `ProviderCapability` 文档（canonical digest 重算）与 declared resolution decision（canonical digest），并把库指纹、pack strategy、`numeric_class`、`deterministic`、reduction order、implementation、provider/artifact 版本常量、provider artifact digest 与 guarantee-detail 家族（`exactness_basis_detail`/`provider_accumulation`/`exactness_envelope`）对账到内嵌真源，再以**注册 provider 身份常量**（`provider_identity.py`：版本/实现/numeric class/reduction order/pack/artifact kind/artifact 版本/AOCL sha256+BLIS tag+arch+ABI 宽度+symbol pin）作外部锚；decision 的 `selected_threads`/`selected_thread_mode` 与 plan-level 线程副本对账；报告内全部 plan-payload 副本（request/policy/candidates/rules/rejections/layout/target/contract/fallback/numeric_class_order/tie_break/declared provider，无 fallback 时的 probe library 字段）逐字节对账内嵌 payload（report-only 改写被具名拒绝）；v3 及更早 report 以 `report_schema_version_legacy_rejected` 明确拒绝，不静默升级。仍未自证的是运行态/实测事实（timing、`artifact.content_digest`、threads 具体值、gates、coverage 计数）、未被代码 pin 的 probe 事实（host features/target snapshot digest、AOCL fingerprint 的 path/size/default_threads 等、vendor manifest 线程计数全量改写）、**内嵌 plan payload 自身的整体协调改写**（payload+digest+重推 decision+全部报告副本，等价于一份"另一个 plan"的完整自洽文档；单改 `resolution.plan_digest`/decision body 已被拒）、以及"整份报告整体替换为另一个注册身份的完整自洽故事"；这些字段**不参与 alias 授权**，完整清单与最小反例见 §7.1。
...
> **report 单文档校验边界（2026-09-12，0042，含 1b/1c 修订；2026-09-13 schema v3 内嵌真源；2026-09-13 schema v4 内嵌 plan payload）**：`validate_report_schema` **只接受 schema v4**；v3 及更早版本以 `report_schema_version_legacy_rejected` 明确拒绝（永不静默升级、也永不按"缺省新检查"解释；`>4` 为 `report_schema_version_mismatch`）。规范集（fail-closed）如下。
>
> **（一）既有规范集**：注册 `OpDefinition` 身份（`resolved.contract.{opcode,version,digest}`）与 `layout_decision` 的 canonical 重推；`requested_policy` 重算 policy digest；跨块 digest/provider 对：`resolution.capability_digest`↔`capability.snapshot_digest`↔`provider_manifest.snapshot_digest`、`resolution.contract_digest`↔`resolved.contract.digest`↔内嵌 manifest 的 contract 条目、`dispatch.provider_declared`↔`resolution.fallback_chain[0]`、`resolved.provider`↔`dispatch.provider_invoked`↔`provider_manifest.provider_id`、`dispatch.fallback_used`↔`resolution.fallback_used`；**全部具名** `resolution.rules[*].details` echo（按 rule name + key 声明式映射；缺失/重复/未知 rule 与未知 detail key 一律拒绝）；`resolution.fallback_chain` 非空、不重复且 `chain[chain_position]` 等于 invoked；selected candidate（按 `state=="selected"` 唯一确定，不看下标）的稳定名锚点（rank/dtype/stride_class/deterministic/threads）；运行时/文档内真源对（`resolution.degraded`、`dispatch.threads_used`、线程模式注册枚举与 `single_thread⇒1`、`declared==invoked` 时 plan-level 模式、`resolved_policy.provider.resolved_scope`、error budget、`guarantees.basis`、portable implementation/`provider_artifact_digest=None`、coverage 的 opcode/provider/artifact_format 副本、portable artifact 版本常量、由注册 contract 推导的 `guarantees.{accumulation,exactness_basis,reference,rounding,scope}` 与 determinism 链）。
>
> **（二）schema v4 内嵌真源与注册身份锚**：report 必须携带 `plan_payload`（plan 的 canonical payload = `resolution.plan_digest` 的 preimage，`digest` 由 body 的 canonical JSON 逐字节重算）、`provider_manifest`（invoked provider 的 `ProviderCapability.to_dict()` 文档 + 由 body 重算的 canonical digest + capability snapshot digest）与 `resolution_decision`（declared/selected 决策，必须是内嵌 `plan_payload.document` 的确定性投影，见 `decision.py`）；任一 digest 与 body 不一致即拒绝。`resolution.plan_digest` 必须等于内嵌 payload 的 digest，decision 的逐字段必须等于由该 payload 的重推结果——因此"只改 `resolution.plan_digest`/decision body + 重算 decision digest"（verify-0043 残余 3 与 R5-full）被拒。内嵌 invoked manifest 由 `provider_identity.py` 从**代码/registry 常量**重建并逐字段比对：provider id、version、implementation（portable 按 opcode 取注册 id）、numeric class、determinism、reduction-order id、pack strategy+`included_in_timing`、artifact kind、dtypes/rank/stride/preconditions、contract digest，以及线程模型的结构/knob/env 账目（`threads.model/knob/env_knobs/api`）；AOCL 另校验 sha256 pin、BLIS tag/commit、arch id/string、ABI 宽度、build options、symbol 列表与重算的 `provider_artifact_digest`；**线程计数值**（`max_threads`、`threads.{max,default}_threads`、`library.default_threads`）是 probe 事实（BLIS 载入线程数 / `os.cpu_count()`），无代码常量可 pin，只要求四者一致且为正——部分协调改写（只改 `max_threads`+`threads.max_threads`）被 `report_provider_manifest_thread_facts_mismatch` 拒绝，四者连同 artifact digest/decision/report 副本的全量改写属残余边界（见四.②）；portable manifest 直接由 `OpDefinition` registry 重建。declared decision 的 `selected_*`/`artifact_*` manifest 事实同样对账到 declared provider 的注册身份；无 runtime fallback 时还与内嵌 invoked manifest（含 probe 相关 library 字段）一致。**manifest-backed 副本对账**：`resolved.{provider,provider_version,implementation,library,numeric_class}`、`artifact.{kind,provider_version,library,pack.strategy,pack.included_in_timing,provider_artifact_digest}`、注册的 lowering/runner/wire/abi 版本常量、`guarantees.{numeric_class,precision_class,deterministic,reduction_order,reduction_order_id}`、`timing.timing_basis.pack_semantics`，以及 **guarantee-detail 家族** `guarantees`/`resolved.{exactness_basis_detail,provider_accumulation,exactness_envelope}`（portable 的空 envelope 由注册 contract 推导为 `exact_contract_bound`）必须等于内嵌真源；**plan-payload 副本对账**：`request`（op/shapes/dtype/policy_digest）、`requested_policy`、`resolved_policy`（含 `fallback.{chain,primary,used,degraded}`）、`policy_applicability`、`layout_decision`（`alias_check` 为运行时覆盖，其余逐字段）、`resolution.{candidates,rejections,rules,numeric_class_order,tie_break,fallback_chain,fallback_primary}`、`dispatch.provider_declared`、`resolved.target`/`capability` 对 payload target、`resolved.contract` 对 payload contract、无 runtime fallback 时的 probe 相关 `resolved.library`/`artifact.library` 必须逐字节等于 `plan_payload.document` 的对应内容——只改报告副本（不动 payload/digest）一律拒绝（`report_numeric_class_order_not_reconciled`、`report_tie_break_not_reconciled`、`report_fallback_chain_not_reconciled`、`report_fallback_primary_not_reconciled`、`report_fallback_degraded_not_reconciled`、`report_plan_payload_copy_not_reconciled`），这是 verify-0044 claim-5 missing class 的收口；`resolution.plan_digest` 与 decision 文档副本对账；decision 的 `selected_threads`/`selected_thread_mode` 与 `resolved_policy.resources.{resolved_threads,resolved_thread_mode}` 及 selected candidate 的 threads echo 对账，无 runtime fallback 时再由内嵌 manifest + digest-checked `requested_policy` 重推。因此"改写全部报告副本（含内嵌文档与重算 digest）"对上述字段会被注册身份锚拒绝；verify-0043 提出的 R1（`exactness_basis_detail`）、R2（`provider_accumulation`）、R3（int8 envelope 常量）三类"可校验但未校验"缺口已按此项闭合。
>
> **（三）说明层**（非规范、不构成执行门）：位置型 `checks[*].detail` 与 `rejections[*].failed_checks[*].detail`、`resolution.why`、coverage 的 count/flag/`parent_opcode`、`dispatch.*_reconciled`、`gates.*`、`timing.timing_basis` 自由文本（`pack_semantics` 已是规范）、`artifact.content_digest`、`thread_state.loaded_threads`、非 selected provider 的 candidate `provider_version`/`numeric_class`/`deterministic` echo、`fallback_events[*]` 逐事件 payload、`tie_break_key` 前缀、`resources.parallelism` 线程旋钮与 `thread_state` 线程环境账目。注意：`resolution.candidates`/`rules`/`rejections` 及其位置型 payload 属于 plan payload 的逐字节副本（report-only 改写已被（二）拒绝），但它们**不是独立重推**的语义事实——payload 整体协调改写仍可替换它们（见（四）③）。
>
> **（四）不可自证的残余边界（完整清单；R1–R3、R5-full、plan-digest/decision 自导出与 report↔plan_payload 副本对账均已闭合；R4/R5 的全量协调改写登记在此）**：① 运行态/实测事实：`timing.*` 数值、`artifact.content_digest`、`dispatch.threads_used` 的具体值（只对账文档内副本一致性）、coverage 计数、`gates.*`、resource accounting、`fallback_events[*]` 的运行时事件与 `resolution.why`；② 未被代码 pin 的 probe 事实：host capability `features`/target snapshot digest、AOCL fingerprint 的 `path`/`name`/`size_bytes`/`default_threads`/BLIS 版本后缀，以及 vendor manifest 线程计数值——四份线程事实一致且为正的校验拒绝 verify-0043 `ce3_max_threads.py` 的局部反例（`report_provider_manifest_thread_facts_mismatch`），但四者连同重算 `provider_artifact_digest`、内嵌 `plan_payload`/decision 与全部报告副本的全量改写仍被接受（数值本身无代码常量）；③ **内嵌 plan payload 未外部锚定**：只改报告副本、只改 `resolution.plan_digest`/decision body、或二者协调但不改 payload 均已被拒（具名码见（二））；但把 payload 一并协调改写（payload body+digest+`resolution.plan_digest`+重推 decision+decision digest+全部报告副本，等价于一份"另一个 plan"的完整自洽文档）仍被接受——这是 request dtype/shape、policy、candidates/rules/rejections、fallback 事实与位置型 trace payload 唯一能移动的路径；④ **注册身份整体替换 / 无执行证明**：锚只证明内嵌文档是某个注册 provider 的 canonical manifest，不证明实际运行的就是该身份；整份报告（全部副本、三个内嵌文档与 digest）整体改写为另一个注册 provider 或另一份自洽 plan 的故事仍可通过（需签名/attestation 才能收，另立任务）；⑤ 说明层中没有 plan-payload 对应物的字段（见（三））：`resolution.why`、`dispatch.*_reconciled`、gates 叙述、coverage counts/flags、线程环境账目等。除以上五类外，报告内其余 manifest-backed / plan-derived 字段均已被（一）（二）对账或在协调改写下被注册身份锚/常量检查拒绝。因此 `validate_report_schema` **不是执行授权**：它在 `ExecutionReport.__init__` 中于执行**之后**运行，仓内没有任何执行路径消费 report，下游消费者不得据 report 重新规划。执行授权只以 `execute_matmul_plan` 对 digest-checked capability 的 canonical 重算（`entry._verify_plan_binding`）通过为准。
