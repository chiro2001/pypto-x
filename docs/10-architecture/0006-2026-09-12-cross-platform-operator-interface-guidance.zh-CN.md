...
> **边界（2026-09-12 独立验收登记，2026-09-13 report schema v3 修订后仍可接受）**：① `ExecutionPlan.validate` 是 **content-local 门**，通过它**不等于**执行授权；执行授权只以执行层对已注册 contract/capability 的 canonical 重算为准（全自洽的 obligation-swap plan 会在 provider 调用前被拒、输出无副作用）。② report 自 **schema v3** 起内嵌 invoked provider 的 `ProviderCapability` 文档（canonical digest 重算）与 declared resolution decision（canonical digest），并把库指纹、pack strategy、`numeric_class`、`deterministic`、reduction order、implementation、provider/artifact 版本常量与 provider artifact digest 对账到内嵌真源，再以**注册 provider 身份常量**（`provider_identity.py`：版本/实现/numeric class/reduction order/pack/artifact kind/artifact 版本/AOCL sha256+BLIS tag+arch+ABI 宽度+symbol pin）作外部锚；v2 report 以 `report_schema_version_legacy_rejected` 明确拒绝，不静默升级。仍未自证的是运行态/实测事实（timing、`artifact.content_digest`、threads 具体值、gates、coverage 计数）、未被代码 pin 的 probe 事实（host features/target snapshot digest、AOCL fingerprint 的 path/size/default_threads 等）、`resolution.plan_digest`（plan payload 未内嵌）与"整份报告整体替换为另一个注册身份的完整自洽故事"；这些字段**不参与 alias 授权**，准确清单见 §7.1。
...
>
> **（四）不可自证的残余边界**（schema v3 仍未闭合，fail-closed 之外必须如实登记）：① 运行态/实测事实：`timing.*` 数值、`artifact.content_digest`、`dispatch.threads_used` 的具体值（只对账文档内副本一致性）、coverage 计数、`gates.*`；② 未被代码 pin 的 probe 事实：host capability `features`/target snapshot digest、AOCL fingerprint 的 `path`/`name`/`size_bytes`/`default_threads`/BLIS 版本后缀；③ `resolution.plan_digest`：plan payload 未内嵌，decision digest 不是其 preimage，协调改写 `resolution.plan_digest` + decision body + 重算 decision digest 仍被接受（单副本与"只改 digest"被拒）；④ **注册身份替换**：锚只证明内嵌文档是某个注册 provider 的 canonical manifest，并不证明实际运行的就是该身份——把整份报告（全部副本、两个内嵌文档与 digest）整体改写为另一个注册 provider 的完整自洽故事仍可通过；⑤ request dtype/shape 等文档内无外部真源字段的全副本协调改写。因此 `validate_report_schema` **不是执行授权**：它在 `ExecutionReport.__init__` 中于执行**之后**运行，仓内没有任何执行路径消费 report，下游消费者不得据 report 重新规划。执行授权只以 `execute_matmul_plan` 对 digest-checked capability 的 canonical 重算（`entry._verify_plan_binding`）通过为准。
