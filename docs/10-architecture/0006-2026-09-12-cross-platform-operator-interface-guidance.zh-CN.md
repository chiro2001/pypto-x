...
> **边界（2026-09-12 独立验收登记，可接受）**：① `ExecutionPlan.validate` 是 **content-local 门**，通过它**不等于**执行授权；执行授权只以执行层对已注册 contract/capability 的 canonical 重算为准（全自洽的 obligation-swap plan 会在 provider 调用前被拒、输出无副作用）。② report 单文档无法对账 vendor manifest-only 字段（库指纹、pack strategy 具体值、vendor implementation id/provider artifact digest/artifact 版本、`guarantees.reduction_order`/`reduction_order_id` 等）与“全副本一致改写”的 request/capability 事实：这些字段**不参与 alias 授权**；但 `guarantees.exactness_basis` 已由 invoked provider 家族常量重算，`guarantees.deterministic` 由 determinism 链/requested 约束部分对账，运行时线程数/模式、`resolution.degraded`、error budget、resolved scope 等文档内副本已按 §7.1 收口；建议后续把 selected provider 的 manifest/capability digest 嵌入 report（follow-up）。
