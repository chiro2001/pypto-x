...
> **report 单文档校验边界（2026-09-12，0042，含 1b 修订）**：`validate_report_schema` 的**规范集**（fail-closed）是：注册 `OpDefinition` 身份（`resolved.contract.{opcode,version,digest}`）与 `layout_decision` 的 canonical 重推；`requested_policy` 重算 policy digest；`resolution.capability_digest`↔`capability.snapshot_digest`、`resolution.contract_digest`↔`resolved.contract.digest`、`dispatch.provider_declared`↔`resolution.fallback_chain[0]`、`resolved.provider`↔`dispatch.provider_invoked`、`dispatch.fallback_used`↔`resolution.fallback_used`；**全部具名** `resolution.rules[*].details` echo（按 rule name + key 声明式映射到 request/capability/target/policy/layout/candidates 的权威副本，缺失/重复/未知 rule 一律拒绝）；`resolution.fallback_chain` 非空、不重复且 `chain[chain_position]` 等于 invoked；selected candidate 的 `rank.requested` 与 `dtype.declared` 锚点（dtype 成员性用 selected provider 的声明域）；coverage 的 opcode/provider/artifact_format 副本；portable artifact 版本常量；以及由注册 contract 推导的 `guarantees.{accumulation,exactness_basis,reference,rounding,scope}` 与 determinism 链。**说明层**（非规范、不构成执行门）：`resolution.candidates[*].checks[*]` 的 `detail`（selected candidate 的 rank/dtype 锚点除外）与 `rejections[*].failed_checks[*].detail` 等位置型 payload、`resolution.why`、coverage 的 count/flag/`parent_opcode`、`dispatch.*_reconciled`、`gates.*`、`timing.timing_basis.*` 自由文本、`artifact.content_digest`、`resolution.plan_digest`、`resolution.degraded`、`thread_state.loaded_threads`、非 selected provider 的 `deterministic`，以及 manifest-only 事实（库指纹、pack strategy 值、vendor artifact 版本、`guarantees.reduction_order`/`reduction_order_id`）。**不可约的单文档极限**：改写一个"在文档内没有外部真源"的事实的**全部副本**（request dtype/shape、capability features、manifest-only 事实）仍可得到自洽 report；因此 **`validate_report_schema` 不是执行授权**，它在 `ExecutionReport.__init__` 中于执行**之后**运行，仓内没有任何执行路径消费 report，下游消费者不得据 report 重新规划。执行授权只以 `execute_matmul_plan` 对 digest-checked capability 的 canonical 重算（`entry._verify_plan_binding`）通过为准。若要求 report 单文档自证，应在 report 内嵌 selected provider 的 manifest（`ProviderCapability` 结构 + canonical digest）并做 fail-closed 校验。
