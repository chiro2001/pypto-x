...
> **边界（2026-09-12 独立验收登记，可接受）**：① `ExecutionPlan.validate` 是 **content-local 门**，通过它**不等于**执行授权；执行授权只以执行层对已注册 contract/capability 的 canonical 重算为准（全自洽的 obligation-swap plan 会在 provider 调用前被拒、输出无副作用）。② report 单文档无法对账 vendor manifest-only 字段的**外部真值**（库指纹、pack strategy 具体值、vendor implementation id/provider artifact digest/artifact 版本、`guarantees.reduction_order`/`reduction_order_id` 等）与“全副本一致改写”的 request/capability 事实：这些字段**不参与 alias 授权**；但 `guarantees.exactness_basis` 已由 invoked provider 家族常量重算，`guarantees.deterministic` 由 determinism 链/requested 约束部分对账，运行时线程数/模式、`resolution.degraded`、error budget、resolved scope 等文档内副本已按 §7.1 收口；文档内副本（`artifact.pack.strategy`↔`timing.timing_basis.pack_semantics`、`artifact.library`↔`resolved.library`）仍强制对账，见 §7.1；建议后续把 selected provider 的 manifest/capability digest 嵌入 report（follow-up）。
...
> **report 单文档校验边界（2026-09-12，0042，含 1b/1c 修订）**：`validate_report_schema` 的**规范集**（fail-closed）是：注册 `OpDefinition` 身份（`resolved.contract.{opcode,version,digest}`）与 `layout_decision` 的 canonical 重推；`requested_policy` 重算 policy digest；`resolution.capability_digest`↔`capability.snapshot_digest`、`resolution.contract_digest`↔`resolved.contract.digest`、`dispatch.provider_declared`↔`resolution.fallback_chain[0]`、`resolved.provider`↔`dispatch.provider_invoked`、`dispatch.fallback_used`↔`resolution.fallback_used`；**全部具名** `resolution.rules[*].details` echo（按 rule name + key 声明式映射，缺失/重复/未知 rule 与未知 detail key 一律拒绝）；`resolution.fallback_chain` 非空、不重复且 `chain[chain_position]` 等于 invoked；selected candidate（按 `state=="selected"` 唯一确定，不看下标）的稳定名锚点：凡带 `rank` check 的候选其 `rank.requested` 必须等于 request rank（declared/invoked 必须带该 check），declared/invoked 的 `dtype.declared` 成员性，selected 的 `stride_class.declared/requested`↔`layout_decision.stride_class`/`contiguous_only`，selected 的 `deterministic.detail.value`↔`guarantees.deterministic`；运行时/文档内真源对：`resolution.degraded == bool(resolution.fallback_events)`，`dispatch.threads_used` 为正整数且等于 invoked candidate 的 `checks[threads]` 与 `intra_op_threads`/`thread_state.effective_threads`/`requested_threads`，线程模式为注册枚举、`single_thread`⇒1 线程、两处 runtime 模式互等，且 `declared==invoked` 时 plan-level 模式等于 runtime 模式（fallback 时 selected provider 的精确 manifest 默认模式仍为边界）；`resolved_policy.resources.resolved_threads`↔selected candidate 的 `checks[threads]`；`resolved_policy.provider.resolved_scope`↔selected candidate scope；`resolved_policy.numeric.error_budget_present` 与 `guarantees.error_budget` 由 `requested_policy.numeric.error_budget` 推导；`guarantees.basis` 由 invoked provider 家族常量决定；invoked 为 portable 时 `resolved.implementation` 等于注册 opcode implementation id 且 `artifact.provider_artifact_digest is None`；coverage 的 opcode/provider/artifact_format 副本；portable artifact 版本常量；以及由注册 contract 推导的 `guarantees.{accumulation,exactness_basis,reference,rounding,scope}` 与 determinism 链。**说明层**（非规范、不构成执行门）：仅未被上述稳定名锚定的位置型 `checks[*].detail` 与 `rejections[*].failed_checks[*].detail`、`resolution.why`、coverage 的 count/flag/`parent_opcode`、`dispatch.*_reconciled`、`gates.*`、`timing.timing_basis.*` 自由文本、`artifact.content_digest`、`resolution.plan_digest`、`thread_state.loaded_threads`、非 selected provider 的 `deterministic`，`fallback_events[*]` 的逐事件 payload、`tie_break_key` 前缀（仅末位 candidate id 被锚定）、`resources.parallelism` 的 `worker_count`/`oversubscription_policy`/`cpu_affinity` 与 `thread_state` 的线程环境账目，以及 manifest-only 的**值-vs-外部真源**事实（vendor implementation id/provider artifact digest/artifact 版本、`guarantees.reduction_order`/`reduction_order_id`、fallback 场景 selected provider 的精确 resolved thread mode）。注意：`artifact.pack.strategy`↔`timing.timing_basis.pack_semantics`（`report_pack_semantics_mismatch`）与 `artifact.library`↔`resolved.library`（`report_artifact_library_not_reconciled`）属于**文档内对被强制对账**的项，只有它们与外部 manifest 的比对才是边界。**不可约的单文档极限**：改写一个“在文档内没有外部真源”的事实的**全部副本**（request dtype/shape、capability features、manifest-only 事实）仍可得到自洽 report；因此 **`validate_report_schema` 不是执行授权**，它在 `ExecutionReport.__init__` 中于执行**之后**运行，仓内没有任何执行路径消费 report，下游消费者不得据 report 重新规划。执行授权只以 `execute_matmul_plan` 对 digest-checked capability 的 canonical 重算（`entry._verify_plan_binding`）通过为准。若要求 report 单文档自证，应在 report 内嵌 selected provider 的 manifest（`ProviderCapability` 结构 + canonical digest）并做 fail-closed 校验。
