...
PyPTO-X（PyPTO Cross-Architecture）是本项目的工作名称，目标是在保留 PyPTO Tensor/Professional 双前端定位的前提下，抽取可移植 Core IR 与 target ABI，并逐步支持鲲鹏 CPU、x86_64 CPU、NVIDIA GPU 和 AMD GPU。项目已进入分波次执行阶段；当前交付仍以架构、治理和可复现基线为主，新后端能力以各波次验收结果为准。
