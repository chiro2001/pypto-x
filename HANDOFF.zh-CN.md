...
状态：`W8A_BF16_ALIGNED_EN_ZH_CHAT_T18_NEAR_TIE_PASS_W8H_W8I_VERIFIED_W8B_B1_B2_B3A_B3B_B3C_B5_B6_VERIFIED_W8C_C1_C2_C3_C4_C5_C6_VERIFIED_W8C_W8A8_GRAPH_PATH_VERIFIED_W8J_J1_J2_VERIFIED_ERR_0003_0004_0005_0006_0007_0008_0009_0010_KF_0001_CLOSED_A3_ONBOARDED_N1_N5_PAUSED_VENDOR_GEMM_ADOPTED_Q1_Q2_Q3_Q4_Q5_Q6_Q7_Q8_Q8B_Q9_Q10_N4_C6_C7_DONE_W8A8_V2_M2_DONE_VIEW_MODE_REQUIRE_STABLE_U2A_VENDOR_PROVIDER_DONE_AVX2_LAYOUT_DONE_SVE256_W8A8_THROUGHPUT_FIXED_0040_AND_0041_AND_0042_AND_0043_PUSHED_0044_PUSHED_REPORT_SCHEMA_V4_MEMO_ELF_INTEGRITY_0046_PUSHED_ALL_VERIFICATIONS_TERMINAL_U3_DISCOVERY_U4_ENV_REGISTRY_CUDA_SM120_OP_BENCH_HARDENED_0045_CUDA_REVIVED_ALL_LEGS_PASS_ERR_0013_0047_INT8_EXACT_BLOCKED_AND_X86_POSITION_NATIVE_LANDED_ERR_0014`

最后更新：2026-09-13 20:40 CST（Asia/Shanghai；**0047 四切片已落地**：冻结 `integration = 0b5b5899e`（tree `412a8b109…`）、补丁 **274**（`am` 复算 274/274 干净、复算 tree = 冻结 tree）、父方 gate **rc=0，2179 passed / 7 skipped / 0 failed，1313.71 s**（collect **2186**）。0047 = ① **int8 可证明包络（exact-blocked）**：K 分块使 pinned AOCL 内核内所有 int32↔f32 往返恒等 + 任意精度累加 + int32 校验 ⇒ `exact_contract_bound`/偏差 0，界**充分且紧**（all-127：K=1040 偏差 0、**K=1041 偏差 1**；分块后 K=133144 偏差 0），默认策略零变化；② **AVX2 位置控制面原生**（payload 6→7/ABI :8，iota/compare/where，语料 raw sha256 11/11 一致）；③ **AVX-512 同缺口补齐**（payload 7→8/ABI :9，跨树 45 cases 不一致 0）；④ **U4 收尾**（登记 6 个仓库范围未登记 env，扫描 33/33 归零，0014 归因订正见 **ERR-0014**）。三条已回判决均为 **PASS_WITH_BOUNDARIES**（int8 `7095f221` / AVX2 `32165fae` / U4-followup `9e0de13f`），AVX-512 验收 `97503c56` 在跑；CUDA 计时腿守望已延长至 96 次窗口（仍全脏，零数字）。0046 与更早结论见下文）**0046 已收口并自动推送**：冻结 `integration = 393d28e0f`（tree `eebb0d584…`）、补丁 **269**（am 复算 269/269 干净、复算 tree = 冻结 tree）、规则 8 全量 **rc=0，2052 passed / 7 skipped / 0 failed，1013.32 s**（collect **2059**，基线 `2d6e87c7d` = 1952）。0046 = ① **U3 发现命令**（doctor/explain/plan + 严格 snapshot I/O + CLI，`DISCOVERY_SCHEMA_VERSION=1`）：独立验收两轮 FAIL（初检 7 条、r2 一个「清空注册 provider 集合」同族窄洞），round-3 修毕后父方以**自建 harness** 收口 10/10 拒绝（验收方复跑脚本硬编码自身 `scripts/` 路径会静默打旧树 → **ERR-0013**）；② **U4 环境变量登记**（40 行 registry = 冻结迁移 35 + 扫描新增 2 + 线程 2 + 控制 1，兼容层零行为变化 + 一次性弃用/未登记告警，5 个 `affects_artifact_semantics` 全在已注册 provider 之外、未发现真实执行层 bug）；③ **CUDA sm_120/sm_121 显式原生目标**（默认 sm_80/PTX 8.0 逐字节不变，显式选择升 payload v3 + `arch_selection`；真机三路径逐位一致、bf16 整图 digest 相同；计时腿 8/8 门检脏 → `perf deferred` 零数字）；④ op-bench 抖动**负载感知**加固（严格规则不变）。三条任务级判决均已终态：U3 FAIL→FAIL→父方收口、CUDA PASS_WITH_BOUNDARIES→PASS、memo PASS_WITH_BOUNDARIES（0044 残余）。0044 与更早结论见下文；0045 CUDA 复活见 §2）。**0039–0043 已全部推送**（`origin/main = a65a3b9`；推送政策自 2026-09-13 起为**自动**）；**0044 已本地收口（两条独立验收均已回，round-2/3 后冻结 T14）**：冻结 **`2d6e87c7d`**（tree `538367cf8…`）、补丁 **258**（am 复算 258/258 干净、复算 tree = 冻结 tree）、规则 8 全量 **rc=0，1945 passed / 7 skipped / 0 failed，907.17 s**（collect **1952**，基线 `0a63ed4dd` = 1921）。0044 = ① report schema **v4**：内嵌必需块 `plan_payload`（`resolution.plan_digest` 的 preimage），digest 三处互等、`resolution_decision` 必须是 payload 的投影（新 leaf 模块 `decision.py`），版本 3→4 fail-closed —— 0043 验收残余 3 与 R5-full 已闭合，新登记残余 3'（完全协调的 alternate payload）与 4（无 attestation 的整份替换）；② SVE256 W8A8 **memo 命中盘上 ELF 完整性**：每次命中 re-hash 对比 `content_digest`（hit 0.642 ms vs miss 2.644 s = 4,119×，整网 QEMU smoke +0.1%），登记 check→exec TOCTOU 等边界（均为本机 QEMU，非 A3 原生）。CUDA 线可用：GamePC 5080 + nvcc 13.3.73 + cuBLAS，见 ERR-0012）
...
4. 用户说"继续"时的默认动作（按优先级）：
   ...
   d) **0042（已本地收口，待用户批准推送；冻结 integration `94d1ab162`，tree `e923e6462…`）**：
      ...
      · **切片① report 校验补强（1/1b/1b2/1c）**：4 条 cross-block + 2 条 contract-derived，随后按 **rule 名**对账全部具名
        ...
        验收 `cb32c1ba` 两轮 **PASS_WITH_BOUNDARIES**：单叶"单点篡改被接受"数 portable 335→110、vendor-f32 347→126、fallback 404→174；
        真 portable + 真 AOCL vendor bf16/f32 + 真 runtime-fallback 报告全部通过（无误伤）。**边界**：协调改写全部副本、`plan_digest`、
        `content_digest`、manifest-only 事实仍不可单文档自证；执行授权只认 `execute_matmul_plan` canonical 重算；下游不得据 report 重新规划。
      · **切片② U2b int8 vendor**：新契约 `qmatmul_s8s8_s32`（int8×int8→int32，rank 2–4，K≤133144，码值 [-127,127]，packed `[K,out]`
        直供 LPGEMM，适配器层 `-128`/K≥133145 fail-closed）。验收 `71599188` 判 **FAIL 仅包络**、其余 7 条 VERIFIED；
      ...
      · **排队**：U5（graph/region，等 N4）、report 签名/attestation（收 0044 残余 4）、
   ...
   f) **0046（已收口并自动推送；冻结 `integration = 393d28e0f`，tree `eebb0d584…`，补丁 269）**：
      · **U3 发现命令**：`doctor`（只读事实 + 8 类 check / 11 行 fail-closed + registry 白名单 env，默认 rc=0、`--strict` 在 fail_closed 时 rc=2/3）、`explain`（直接复用 resolver trace：候选拒绝原因/tie-break/fallback/guarantees/边界）、`plan`（冻结 plan + capability snapshot digest，可重放）、严格 snapshot I/O 与 `python -m pypto.execution.cli`；契约文档 `docs/20-planning/0013`。独立验收 `72415831` 两轮 FAIL（初检 7 条 → r2 一个「清空注册 provider 集合」同族窄洞）→ round-3 修毕、父方**自建 harness** 10/10 拒绝收口；验收方复跑脚本硬编码 `…/verify-*/scripts` 会静默加载旧树（**ERR-0013**），此类结论必须以实际加载树为准。
      · **U4 环境变量登记**：`env_registry.py`（`ENV_REGISTRY_SCHEMA_VERSION=1`）40 行 = 冻结迁移 35（22 deprecated + 13 internal）+ 扫描新增 2 + 线程 2 + 控制 1；35/35 有静态调用点锚点、`unregistered_reads: []`；10 变量 plan digest 不变、线程变量改 capability digest → 跨环境 replay 结构化 fail-closed；5 个 `affects_artifact_semantics` 全在已注册 provider 之外（无真实执行层 bug、无最小修复）；旧变量行为零变化 + 进程一次性弃用/未登记告警（可静默、敏感名脱敏）；文档 `docs/20-planning/0014`。**独立验收在推送后补派**（`_meta/pypto-x/verify-0046-u4/`），判决回后按批次惯例 follow-up 记录。
      · **CUDA sm_120/sm_121**：显式可选一等 arch（默认仍 sm_80/PTX 8.0 且逐字节不变；显式选择升 payload v3 + `arch_selection` 被 digest 覆盖）；真机三路径（sm80_ptx/sm120_ptx/sm120_cubin）逐位一致，bf16 整图输出 digest 相同 `c36ea307…`；独立验收 `2bda30dc` PASS_WITH_BOUNDARIES（6 条 fail-safe 边界）→ r2 **PASS**（D1–D6 全闭合）；计时腿 8/8 门检脏 → `perf deferred`（**零数字**，命令存 `cuda-sm120/compare/PERF_DEFERRED.md`，干净窗口可补测）。
      · **op-bench 抖动加固**：负载感知（最多 3 对、任一对过严格判据即过；仅当全失败且 `load1 > max(4, 0.25·nproc)` 或 `cpu PSI avg10 > 2` 才接受 ≤50% 漂移并打印理由；≥50% 或结构不符一律失败；锁内 `repeat=2,R=21` 严格规则不变）。
      · 规则 8 全量 **rc=0 / 2052 passed / 7 skipped / 0 failed / 1013.32 s**（collect 2059，基线 1952）；补丁 **269**（am 复算 269/269 干净、复算 tree = 冻结 tree）；证据 `_meta/pypto-x/{u3-discovery,u4-env-migration,cuda-sm120,batch-0046-u3-cuda-sm120,verify-0046-*}`。
   g) **0047（四切片已落地，三判已回、AVX-512 验收在跑；冻结 `integration = 0b5b5899e`，tree `412a8b109…`，补丁 274）**：
      · **int8 可证明包络（exact-blocked）**：新 opt-in provider `vendor:aocl-lpgemm-int8-exact-blocked`，`kc = min(2048, floor(2²⁴/(max|a|·max|b|)))` 分段调用 pinned `aocl_gemm_s8s8s32os32`（alpha=1/beta=0/post_op=None），段内 int32↔f32 往返因 |值|≤2²⁴ 恒等，段结果 Python 任意精度累加 + int32 范围校验 ⇒ `class=exact`/`exact_contract_bound`/`all_k_le_limit: 0`；界**充分且紧**（all-127：K=1040 偏差 0、**K=1041 偏差 1**；分块后 K=16696/K=133144 偏差 0）。默认策略零变化（不在 default/allowed，60 条非 opt-in 两树逐字段相等；legacy 逐字节不变）。验收 `7095f221` = PASS_WITH_BOUNDARIES（336 构造差分 + **1307 例 falsification** 无界内反例）。
      · **AVX2 位置控制面原生（payload 6→7、ABI :8、`v1:pxio-pxcp-pxwh+fnv1a64`）**：iota/compare/where 全原生、`host_preexpanded_operations=[]`、rank>16 带因回退；11 用例 Qwen 语料 raw sha256 11/11 一致；性能 5.7×–75.2×（**UNGATED**）。验收 `32165fae` = PASS_WITH_BOUNDARIES（oracle 未改 + 第二独立 oracle 184 checks，契约两处措辞已订正）。
      · **AVX-512 位置控制面原生（payload 7→8、ABI :9）**：同缺口补齐；跨树 45 cases 不一致 0；`python_list_elements==0` 硬断言；只依赖 AVX-512F（证据性声明）。验收 `97503c56` 在跑。
      · **U4 收尾**：登记 6 个仓库范围未登记 `PYPTO_X_*`（schema 仍 v1），仓库范围扫描 33/33 已登记；`affects=true` 全 6 行补筛 37 项 `all_equal`；doctor 40→46；验收 `9e0de13f` = PASS_WITH_BOUNDARIES（唯一在范围问题为 0014 归因 → **ERR-0014**）。
      · 父方 gate：**rc=0 / 2179 passed / 7 skipped / 0 failed / 1313.71 s**（collect 2186）；补丁 **274**（`am` 复算 274/274 干净、复算 tree = 冻结 tree）；证据 `_meta/pypto-x/{int8-exact-blocked,avx2-position-native,avx512-position-native,u4-followup-registry,batch-0047-int8-exact-and-x86-position,verify-0047-*}`；快照 `docs/00-handoffs/0047-2026-09-13-int8-exact-blocked-and-x86-position-native.zh-CN.md`。
...
7. 提交纪律：公开主仓 = 本目录（origin）；改完实现 → cherry-pick 到 integration → 独立 verify → 更新
   ...
   （0038 收口已 push 到 7a064ab；**0039 批次及之后已 push 到 7857ae6，补丁 144**；**0040 批次已推送**（`7857ae6 → 41982db`、补丁 217）；**0041 已推送**（补丁 235）；**0042 已推送**（`0d0ec76 → 4ea914e`、补丁 252、规则 8 1854 passed）；**0043 已推送**（`da35516 → 2d20846`、补丁 254、规则 8 1914 passed）。**推送政策（2026-09-13 起）**：用户明示"后续不要询问，自动推送"→ 批次收口后**自动**执行 `git push origin main` + `scripts/remote/sync_private_backup.sh` 并把 range 与备份状态记进 lock；仍**绝不** force-push、绝不改历史。）
